...
Fl_Image_Display::handle(int ev) // I - Event to handle
{
    float zoom_ = 2.0f;
    int m_x, m_y;
    int X = 0, Y = 0;
    int X0 = 0;
    int Y0 = 0;
    int xw = xsize_, yh = ysize_;
    const int W = w() - SBWIDTH - Fl::box_dw(box());
    const int H = h() - SBWIDTH - Fl::box_dh(box());
    ...
    if (value_)
    {
        ...
        case FL_SHORTCUT :
            ...
            case FL_KP + '-' : // Zoom out
                if (factor_)
                    ...
                    scale((float)xsize_ / (float)value_->w() / zoom_);
        ...
        case FL_PUSH :
            if (Fl::event_x() < (x() + w() - SBWIDTH) && Fl::event_y() < (y() + h() - SBWIDTH))
            {
                update_mouse_xy();
        ...
        */
        ...
        case  FL_MOVE:
            m_x = Fl::event_x() - x() - Fl::box_dx(box());
            m_y = Fl::event_y() - y() - Fl::box_dx(box());
            ...
            if (send_coords)
            {
                (*cb)((int)((m_x - X0) / factor_), (int)((m_y - Y0) / factor_));
            }
                           ...
                           uchar *D)  // O - Image data
{
    ...
    if (xsize > (display->w() - Fl::box_dw(display->box()) - SBWIDTH))
    {
        ...
        X = (X + display->xscrollbar_.value()) * img->w() / xsize;
    }
    else
    {
        //X = X * (img->w() - 1) / (xsize - 1);
    ...
    {
        //Y = (Y + display->yscrollbar_.value()) * (img->h() - 1) / (display->ysize_ - 1);
        Y = (Y + display->yscrollbar_.value()) * img->h() / display->ysize_;
    }
    else
    {
        //Y = Y * (img->h() - 1) / (display->ysize_ - 1);
        Y = Y * img->h() / display->ysize_;
    }
...
Fl_Image_Display::scale(float factor) // I - Scaling factor (0 = auto)
{
    ...
    {
        xsize_ = (int)(value_->w() * factor_);// + 0.5f);
        ysize_ = (int)(value_->h() * factor_);// + 0.5f);
...
Fl_Image_Display::set_gamma(float val)  // I - Gamma value
{
    ...
    for (i = 0; i < 256; i ++)
        gamma_lut_[i] = (int)(255.0f * pow(i / 255.0f, val) + 0.5f);
}
...
void
Fl_Image_Display::value(Fl_Shared_Image *v, float factor)
// I - Image to display
{
    value_ = v;
    scale(factor);
...
void 
Fl_Image_Display::set_callback(void (*fcb)(const int cx, const int cy)) 
{
    cb = fcb; 
}
